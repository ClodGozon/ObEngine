...
namespace obe::Animation
{
    ...
    {
        Debug::Log->trace("<Animator> Clearing Animator at {0}",
    ...
    {
        Debug::Log->debug("<Animator> Loading Animator at {0}",
                          m_animatorPath.toString());
    ...
    void Animator::update()
    {
        if (!m_paused)
        {
            ...
            if (m_target)
            {
                ...
                {
                    m_target->setSize(Transform::UnitVector(
                        m_target->getSize().x, float(texture.getSize().y) /
                                                   float(texture.getSize().x) *
                                                   m_target->getSize().x));
                }
                else if (m_targetScaleMode ==
    ...
    void Animator::setTarget(Graphics::LevelSprite& sprite,
